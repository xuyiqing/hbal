...
From the above plots, we can see that the linear terms (mean) of the covariates are exactly balanced between the treatment and the control groups. We can check the penalties applied to different groups of covariates. In this case, the higher-order terms also have relatively low penalties, and imbalance among higher-order terms is significantly reduced. 
...
**hbal** is an extension of **ebal**. By setting `expand.degree=0` and `cv=FALSE`, which tells `hbal()` to not expand the covariate space and not use cross validation to search for regularization hyperparameters, `hbal()` is equivalent to entropy balancing introduced by @hainmueller2012entropy. 
...
```
We see that the third-order terms are being more heavily penalized by the two-way interactions. 
...
summary(att(out, clusters=contenderJudges$judge))$coefficients['Treat',]
```
